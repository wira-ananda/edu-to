<script lang="ts">
  ...
  async function loadParticipants() {
    ...
    try {
      const result = await apiFetch<TryoutParticipantsResponse>(
  ...
  async function enrollStudent(event: SubmitEvent) {
    ...
    try {
      const result = await apiFetch<EnrollStudentResponse>(
        `/admin/tryouts/${tryoutId}/participants/${cleanedStudentId}`,
        {
          method: "POST",
        },
      );
  ...
  >
    <div>
      ...
      <p class="mt-1 text-sm text-slate-500">
        Kelola siswa yang mendaftar atau sudah menjadi peserta pada tryout ini.
      </p>
    </div>
    ...
    <div class="flex flex-wrap gap-2">
      ...
      <button
        type="button"
        onclick={() => goto(`/admin/results?tryoutId=${tryoutId}`)}
        disabled={!tryoutId}
