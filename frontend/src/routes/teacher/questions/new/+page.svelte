<script lang="ts">
  ...
  async function handleSubmit(event: SubmitEvent) {
    ...
    try {
      ...
      await apiFetch<TeacherMutateQuestionResponse>("/teacher/questions", {
        method: "POST",
        body: formData,
      });
...
<section class="space-y-5">
  <div>
    ...
    <h2 class="text-2xl font-bold text-slate-950">Tambah Soal</h2>

    <p class="mt-1 text-sm text-slate-500">
      Soal yang dibuat akan menjadi milik akun guru ini.
    </p>
  </div>
    ...
    >
      ...
      <div>
        ...
        {#if subjects.length === 0}
          <p class="mt-2 text-xs font-semibold text-red-600">
            Buat bank soal terlebih dahulu dari halaman Bank Soal.
          </p>
        {/if}
      </div>
      ...
      <div class="rounded-2xl border border-slate-200 bg-slate-50 p-4">
        <div class="flex flex-wrap items-center justify-between gap-3">
          <div>
            <p class="text-sm font-bold text-slate-900">Analisis Kesulitan</p>

            <p class="text-xs text-slate-500">
              Analisis otomatis berdasarkan indikator soal.
            </p>
          </div>
      ...
      <div class="flex justify-end gap-3 border-t border-slate-100 pt-5">
        ...
        <button
          type="submit"
          disabled={saving || formInvalid || subjects.length === 0}
